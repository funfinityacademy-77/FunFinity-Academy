...
export default function AdminTeachDashboard() {
  const { user } = useAuth();
  ...
  const pendingSubmissions = (myAssignments || []).reduce((sum: number, a: any) => sum + (a.assignment_submissions?.length || 0), 0);
  ...
  const classStats = [
    ...
    { label: "Assignments", value: String((myAssignments || []).length), icon: ClipboardList, colorClass: "bg-accent/10 text-accent" },
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <motion.div {...fadeIn(0)} className="platform-card p-6 lg:p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-muted-foreground text-sm mb-4">
            {(myCourses || []).length} courses · {totalStudents} total students · {pendingSubmissions} submissions to review
          </p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <motion.div {...fadeIn(0.3)} className="space-y-4">
          <h2 className="font-display text-lg font-semibold text-foreground">Recent Assignments</h2>
          {(myAssignments || []).length === 0 ? (
            <div className="platform-card p-8 text-center text-sm text-muted-foreground">No assignments created yet</div>
          ) : (
            (myAssignments || []).slice(0, 5).map((assignment: any) => (
              <div key={assignment.id} className="platform-card p-4">
                <p className="font-medium text-foreground text-sm">{assignment.title}</p>
                <p className="text-xs text-muted-foreground">
                  {assignment.assignment_submissions?.length || 0} submissions
