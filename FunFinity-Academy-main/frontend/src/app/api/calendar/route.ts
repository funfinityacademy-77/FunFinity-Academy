// Calendar API Route
// Isolated module for calendar functionality
...
export async function GET(request: NextRequest) {
  try {
    ...
    const events = await postgres.query(query, params);

    // Get total count for pagination
    const countQuery = 'SELECT COUNT(*) as total FROM calendar_events WHERE user_id = $1';
    ...
    const countResult = await postgres.queryOne(countQuery, countParams);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const event = await postgres.queryOne(insertQuery, [
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existingEvent = await postgres.queryOne(
    ...
    const event = await postgres.queryOne(updateQuery, [
      validatedData.title,
