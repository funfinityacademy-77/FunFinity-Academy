// Chat API Route - Clean Backend Integration
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'rooms') {
      ...
      const rooms = await database.query(query, params);
      ...
      const messages = await database.query(messagesQuery, [roomId, limit, offset]);
      ...
      const participants = await database.query(participantsQuery, [roomId]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'create_room') {
      ...
      const room = await database.queryOne(roomQuery, [
      ...
      const participant = await database.queryOne(participantQuery, [room.id, validatedData.created_by]);

      // Update participant count
      await database.query('UPDATE chat_rooms SET participant_count = 1 WHERE id = $1', [room.id]);
      ...
      // TODO: BACKEND - Verify user is participant
      const participantCheck = await database.queryOne(
      ...
      const message = await database.queryOne(messageQuery, [
      ...
      await database.query('UPDATE chat_rooms SET updated_at = CURRENT_TIMESTAMP WHERE id = $1', [validatedData.room_id]);
      ...
      // TODO: BACKEND - Check if room exists
      const room = await database.queryOne('SELECT id FROM chat_rooms WHERE id = $1 AND is_active = true', [validatedData.room_id]);
      ...
      // TODO: BACKEND - Check if already participant and join or reactivate
      const existingParticipant = await database.queryOne(
      ...
      if (existingParticipant) {
        ...
        participant = await database.queryOne(reactivateQuery, [validatedData.role, existingParticipant.id]);
      } else {
        // Add new participant
        const joinQuery = `
          INSERT INTO chat_participants (room_id, user_id, role, joined_at)
        ...
        participant = await database.queryOne(joinQuery, [
          validatedData.room_id,
          validatedData.user_id,
          validatedData.role,
        ]);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (action === 'delete_room' && roomId) {
      ...
      // TODO: BACKEND - Verify message ownership
